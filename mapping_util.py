...
# ---------------------------------------------------------
	...
	linkage_nok_schema = db_conf['linkage_nok_schema'] if 'linkage_nok_schema' in db_conf else None	
	linkage_schema = db_conf['linkage_schema'] if 'linkage_schema' in db_conf else None	
	...
	tablespace = db_conf['tablespace'] if 'tablespace' in db_conf else None	
	...
	for idx, item in enumerate(query_list):
		...
		query = query.replace('{LINKAGE_NOK_SCHEMA}', linkage_nok_schema) if linkage_nok_schema is not None else query
		query = query.replace('{LINKAGE_SCHEMA}', linkage_schema) if linkage_schema is not None else query
		...
		query = query.replace('{SOURCE_NOK_SCHEMA_TO_LINK}', source_nok_schema_to_link) if source_nok_schema_to_link is not None else query
		...
		query = query.replace('{TABLESPACE}', tablespace) if tablespace is not None else query
