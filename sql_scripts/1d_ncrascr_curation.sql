...
INSERT INTO {SOURCE_NOK_SCHEMA}.tumour(
	...
	where t2.person_id is null													-- Eliminate patients that do not exist in {TARGET_SCHEMA_TO_LINK}.person
...
alter table {SOURCE_NOK_SCHEMA}.tumour add constraint pk_tumour_nok primary key (patid,cr_patid,cr_id) USING INDEX TABLESPACE pg_default;

DELETE FROM {SOURCE_SCHEMA}.tumour as t1 
using {SOURCE_NOK_SCHEMA}.tumour as t2
WHERE t1.patid = t2.patid
and t1.cr_patid = t2.cr_patid
and t1.cr_id = t2.cr_id;
...
With cte as(																
	...
	group by 	patid, cr_id, cr_patid, number_of_tumours,		
				eventcode, eventdesc, eventdate,				
				within_six_months_flag,	six_months_after_flag, 
				opcs4_code, opcs4_name, 
				radiocode, radiodesc, imagingcode, imagingdesc, imagingsite, lesionsize, 
