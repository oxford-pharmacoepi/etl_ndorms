...
cte2 AS (
	SELECT t2.ins_index, MIN(t2.epistart) AS date_min, MAX(t2.epiend) AS date_max 
	FROM cte1 as t1
	INNER JOIN {SOURCE_SCHEMA}.hesin AS t2 ON t2.eid = t1.person_id
	GROUP BY t2.ins_index
),
cte3 AS (
	SELECT
--	NEXTVAL('{TARGET_SCHEMA}.sequence_vo') AS visit_occurrence_id,
	...
	COALESCE(t1.admidate, t3.date_min, t1.disdate) AS visit_start_date, 
	COALESCE(t1.admidate, t3.date_min, t1.disdate) AS visit_start_datetime,
	COALESCE(t1.disdate, t3.date_max, t3.date_min) AS visit_end_date,
	COALESCE(t1.disdate, t3.date_max, t3.date_min) AS visit_end_datetime,
	...
	INNER JOIN cte1 as t2 ON t1.eid = t2.person_id
	LEFT JOIN cte2 as t3 ON t1.ins_index = t3.ins_index
...
cte2 AS (
	...
	t1.ins_index
	FROM cte1 as t0 
	INNER JOIN {SOURCE_SCHEMA}.hesin_critical AS t1 ON t1.patid = t0.person_id
...
cte5 AS (
	...
	9201 AS visit_detail_concept_id,
	t1.detndate AS visit_detail_start_date,
	t1.detndate AS visit_detail_start_datetime,
	...
	NULL::int AS visit_occurrence_id
	FROM cte1 as t0 
	INNER JOIN {SOURCE_SCHEMA}.hesin_psych AS t1 ON t1.patid = t0.person_id
),
cte6 AS (
	SELECT 
	...
	t1.visit_detail_concept_id,
	CASE 
        WHEN t1.visit_detail_start_date IS NULL THEN t3.admindate
        ELSE t3.epistart
    END AS visit_detail_start_date,
	CASE 
        WHEN t1.visit_detail_start_datetime IS NULL THEN t3.admindate
        ELSE t3.epistart
    END AS visit_detail_start_datetime,	
	
	CASE 
        WHEN t1.visit_detail_end_date IS NULL THEN t3.disdate
        ELSE t3.epiend
    END AS visit_detail_end_date,
	CASE 
        WHEN 	t1.visit_detail_end_datetime IS NULL THEN t3.disdate
        ELSE t3.epiend
    END AS visit_detail_end_datetime,
	t1.visit_detail_type_concept_id,
	t1.provider_id,
	...
	FROM cte5 as t1
	INNER JOIN {TARGET_SCHEMA}.visit_occurrence AS t2 ON t2.visit_source_value = t1.ins_index and t2.person_id = t1.person_id
