...
-- Moved before VISIT_DETAIL so it can be used in there
--with cte1 as (
--	SELECT person_id,
...
--	left join {SOURCE_SCHEMA}.temp_visit_detail t2 on t1.parent_visit_detail_id = t2.visit_detail_source_id
--ANTO	and t2.source_table = 'Observation' -- This condition wasn't in Clair's etl, but it avoids garbage duplication of rows where parentobsid = consid
