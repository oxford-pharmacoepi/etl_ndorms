...
cte2 AS (
	SELECT t2.attendkey,MIN(t2.apptdate) AS date_min, MAX(t2.apptdate) AS date_max 
	FROM cte1 as t1
	INNER JOIN {SOURCE_SCHEMA}.hesop_appointment AS t2 ON t2.patid = t1.person_id
	GROUP BY t2.attendkey
),
cte3 AS (
	SELECT
	...
	COALESCE(t3.date_max, t3.date_min) AS visit_end_datetime,
	...
	FROM {SOURCE_SCHEMA}.hesop_clinical AS t1
	INNER JOIN cte1 as t2 ON t1.patid = t2.person_id
	LEFT JOIN cte2 as t3 ON t1.attendkey = t3.attendkey
	LEFT JOIN {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t4 on t1.tretspef = t4.source_code and t4.source_vocabulary_id = 'HES_SPEC_STCM'
 	ORDER BY patid, t3.date_min,COALESCE(t3.date_max, t3.date_min)
),
cte4 AS (
