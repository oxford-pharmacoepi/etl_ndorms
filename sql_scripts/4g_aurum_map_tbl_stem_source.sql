...
--	),
--	t1 as (
--		select o.patid, o.staffid as provider_id, o.obsid, o.obsdate as start_date, 
...
----		left join {SOURCE_SCHEMA}.consultation c on o.consid = c.consid	-- AD ORIGINALLY USED FOR c.staffid: why should we use it instead of o.staffid?
----		where o.obsdate is not null  -- AD obsdate is always not null, filtered in check_source_data.sql
--	),
--	 t2 as (
--		select t1.*,
...
--			then t1.start_date
--			else p.probenddate
--		end as end_date, n.description as unit_source_value
--		from t1
--		left join {SOURCE_SCHEMA}.problem p on t1.obsid = p.obsid
--		left join {SOURCE_SCHEMA}.numunit n on t1.numunitid = n.numunitid
--	),
--	t3 as (
--		SELECT t2.*, v.visit_detail_id, v.visit_detail_source_id, vd.visit_occurrence_id
--		FROM t2
--		left join {SOURCE_SCHEMA}.temp_visit_detail v on t2.obsid = v.visit_detail_source_id
--		left join {TARGET_SCHEMA}.visit_detail vd on v.visit_detail_id = vd.visit_detail_id
--		WHERE v.source_table = 'Observation'
--	)
--insert into chunks.stem_source_{CHUNK_ID} (person_id, visit_occurrence_id, visit_detail_id, provider_id, source_value,
...
--		t3.numrangehigh::double precision range_high,
--		t3.numrangelow::double precision range_low,
--		0 route_concept_id,
--		0 unit_concept_id,
--		t3.unit_source_value,
--		0 value_as_concept_id,
--		t3.value value_as_number,
--		t3.value::varchar value_source_value,
--		0 anatomic_site_concept_id,
--		0 disease_status_concept_id,
--		0 modifier_concept_id,
--		'Observation' stem_source_table,
--		t3.obsid as stem_source_id
--from t3
--left join {SOURCE_SCHEMA}.medicaldictionary m on t3.medcodeid = m.medcodeid
--left join {SOURCE_SCHEMA}.temp_concept_map t on t3.medcodeid = t.medcodeid;
