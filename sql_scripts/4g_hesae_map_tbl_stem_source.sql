...
--Inserting Accident & Emergency diagnoses/ICD-10/Read coded diagnosis records into stem_source table from hesae_diagnosis
WITH cte0 AS (
    SELECT person_id
...
cte1 AS (
	SELECT 
		...
		t2.aekey::VARCHAR AS visit_source_value,
		t2.aekey::VARCHAR AS visit_detail_source_value,
		t3.arrivaldate AS start_date,
		t3.arrivaldate AS end_date,
		CASE
			WHEN t2.DIAGSCHEME = 1 THEN CASE WHEN LENGTH(t2.diag) = 2 OR LENGTH(t2.diag) = 3 THEN t2.diag ELSE t2.diag END
			WHEN t2.DIAGSCHEME = 2 THEN
				CASE 
					WHEN LENGTH(t2.diag) = 4 AND RIGHT(t2.diag, 1) IN ('X', '.') THEN LEFT(t2.diag, 3) 
					WHEN LENGTH(t2.diag) = 4 THEN CONCAT(LEFT(t2.diag, 3), '.', RIGHT(t2.diag, 1))
					ELSE t2.diag
				END
			WHEN t2.DIAGSCHEME = 4 THEN f.cleansedreadcode ELSE t2.diag
		END AS source_value, 
		CASE 
	...
	FROM 
		...
		LEFT JOIN source.medicaldictionary f ON LEFT(f.cleansedreadcode, 5) = LEFT(t2.diag, 5)
	ORDER BY 
		t1.person_id, t2.aekey, t2.diag_order
),	
cte2 as (
		...
		WHERE upper(t5.source_vocabulary_id) IN ( 'ICD10','HESAE_DIAG_STCM','READ')
)
