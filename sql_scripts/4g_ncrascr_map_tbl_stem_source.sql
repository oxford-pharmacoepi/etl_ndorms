...
cte1 as (
		...
		t2.morph_icd10_o2, t2.behaviour_icd10_o2, site_icd10_o2, site_coded,
		CASE 
...
cte1a as (
		...
		cr_id, 
		start_date, 
		morph_icd10_o2 || '/' || behaviour_icd10_o2 || '-' || site_icd10_o2 || '.9' as source_value,
		type_source_value
		from cte1
...
cte2b as (
		...
		type_source_value
		from cte1b as t1
		left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 ON (t1.site_coded = replace(t2.source_code,'.','') OR replace(t1.site_coded,'-','') = t2.source_code)
...
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t3 on t1.type_source_value = t3.source_code::numeric 
...
WITH cte0 as (
	...
	join cte0 on cte0.person_id = t1.patid
	join {TARGET_SCHEMA}.observation_period as t2 on t1.patid = t2.person_id
	WHERE t1.diagnosisdatebest >= t2.observation_period_start_date 
	and t1.diagnosisdatebest <= t2.observation_period_end_date
), cte2 as(
	select
		patid,	
		CASE 
...
--	t1.range_high,
	...
	t1.cr_id as stem_source_id
from cte2 as t1
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.cr_id = tt.visit_detail_source_id and tt.source_table = 'Tumour'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 on t2.source_code = t1.source_value 
and t2.source_vocabulary_id = 'NCRAS_TUMOUR_MODIFIER_STCM' 
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t3 on t1.type_source_value = t3.source_code::numeric 
and t3.source_vocabulary_id = 'NCRAS_TUMOUR_BASIS_DIAG_STCM';
...
from cte1 as t1
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.cr_id = tt.visit_detail_source_id and tt.source_table = 'Tumour'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 on t2.source_code = t1.grade 
and t2.source_vocabulary_id = 'NCRAS_TUMOUR_GRADE_STCM'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t3 on t1.type_source_value = t3.source_code::numeric 
and t3.source_vocabulary_id = 'NCRAS_TUMOUR_BASIS_DIAG_STCM';
...
WITH cte0 as (
	...
	from {SOURCE_SCHEMA}.tumour as t1
	join cte0 as t2 on t1.patid = t2.person_id
...
select distinct
	...
	t1.cr_id as stem_source_id
from cte1 as t1
join {TARGET_SCHEMA}.observation_period as t5 on t1.patid = t5.person_id
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.cr_id = tt.visit_detail_source_id and tt.source_table = 'Tumour'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 on t2.source_vocabulary_id = 'Cancer Modifier' 
and UPPER(t2.source_code) = UPPER(t1.stage_system || 'AJCC/UICC-STAGE-'|| t1.source_value) and t2.target_domain_id = 'Measurement' -- for Athena changes target domain during STCM update
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t3 on t3.source_vocabulary_id = 'Cancer Modifier' 
and UPPER(t3.source_code) = UPPER(t1.stage_system || 'AJCC/UICC-STAGE-'|| LEFT(t1.source_value, 1)) and t3.target_domain_id = 'Measurement' 
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t4 on t1.type_source_value = t4.source_code::numeric 
...
select 
	...
	t1.cr_id as stem_source_id
from cte1 as t1
join {TARGET_SCHEMA}.observation_period as t3 on t1.patid = t3.person_id
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.cr_id = tt.visit_detail_source_id and tt.source_table = 'Tumour'
...
from cte1 as t1
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.cr_id = tt.visit_detail_source_id and tt.source_table = 'Tumour'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 on t2.source_code = t1.gleason_primary::text 
and t2.source_vocabulary_id = 'NCRAS_TUMOUR_GLEASON_PRI_STCM' 
and t2.target_domain_id = 'Measurement' -- for Athena changes target domain during STCM update
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t3 on t1.type_source_value = t3.source_code::numeric 
and t3.source_vocabulary_id = 'NCRAS_TUMOUR_BASIS_DIAG_STCM';
...
from cte1 as t1
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.cr_id = tt.visit_detail_source_id and tt.source_table = 'Tumour'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 on t2.source_code = t1.gleason_secondary::text 
and t2.source_vocabulary_id = 'NCRAS_TUMOUR_GLEASON_SEC_STCM' 
and t2.target_domain_id = 'Measurement' -- for Athena changes target domain during STCM update
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t3 on t1.type_source_value = t3.source_code::numeric 
and t3.source_vocabulary_id = 'NCRAS_TUMOUR_BASIS_DIAG_STCM';
...
from cte1 as t1
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.cr_id = tt.visit_detail_source_id and tt.source_table = 'Tumour'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 on t2.source_code = t1.gleason_tertiary::text 
and t2.source_vocabulary_id = 'NCRAS_TUMOUR_GLEASON_TER_STCM' 
and t2.target_domain_id = 'Measurement' -- for Athena changes target domain during STCM update
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t3 on t1.type_source_value = t3.source_code::numeric 
and t3.source_vocabulary_id = 'NCRAS_TUMOUR_BASIS_DIAG_STCM';
...
select 
	...
	t1.cr_id as stem_source_id
from cte1 as t1
inner join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.cr_id = tt.visit_detail_source_id and tt.source_table = 'Tumour'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t3 on t1.type_source_value = t3.source_code::numeric 
and t3.source_vocabulary_id = 'NCRAS_TUMOUR_BASIS_DIAG_STCM';
...
select 
	...
	t1.cr_id as stem_source_id
from cte1 as t1
inner join {SOURCE_SCHEMA}.temp_visit_detail as t2 on t1.cr_id = t2.visit_detail_source_id and t2.source_table = 'Tumour'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t3 on t1.type_source_value = t3.source_code::numeric 
and t3.source_vocabulary_id = 'NCRAS_TUMOUR_BASIS_DIAG_STCM';
...
select 
	...
	tt.visit_occurrence_id,
	tt.visit_detail_id,
	t1.screeningstatusfull_name as source_value,
	0 as source_concept_id,
	COALESCE(t3.target_concept_id, 32879) as type_concept_id,
	t1.diagnosisdatebest as start_date,
	t1.diagnosisdatebest as end_date,
...
select
	...
	t1.treatment_id as stem_source_id
from cte2 as t1
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.treatment_id::varchar = tt.visit_detail_source_id and tt.source_table = 'Treatment'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 on t2.source_code = t1.source_value::text 
...
select  
	...
	t2.source_code_description as value_source_value,
	'Treatment-Modifier' as stem_source_table,
...
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.treatment_id::varchar = tt.visit_detail_source_id and tt.source_table = 'Treatment'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 on t2.source_code = t1.source_value 
and t2.source_vocabulary_id = 'NCRAS_TREATMENT_IMAGING_STCM';
...
from cte2 as t1
join {SOURCE_SCHEMA}.temp_visit_detail as tt on t1.treatment_id::varchar = tt.visit_detail_source_id and tt.source_table = 'Treatment'
left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map as t2 on replace(t2.source_code,'.','') = t1.source_value 
and t2.source_vocabulary_id = 'OPCS4';
	
------------------------------
-- Imagingsite / Imagingcode
------------------------------
WITH cte0 as (
	select person_id
	from {CHUNK_SCHEMA}.chunk_person
