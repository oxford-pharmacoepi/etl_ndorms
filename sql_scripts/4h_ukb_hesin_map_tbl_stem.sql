...
with cte1 as (
	...
	left join {VOCABULARY_SCHEMA}.source_to_standard_vocab_map v
		...
		and s.source_value = v.source_code
		and UPPER(v.source_vocabulary_id) IN ('OPCS4', 'ICD10', 'ICD9CM')
),
cte2 as (
	select case when {CHUNK_ID} = 1 THEN 0 ELSE 		
		(SELECT stem_id_end FROM {CHUNK_SCHEMA}.chunk where chunk_id = {CHUNK_ID}-1) end as start_id
)
insert into {CHUNK_SCHEMA}.stem_{CHUNK_ID} (domain_id, person_id, visit_occurrence_id, visit_detail_id, provider_id, id, concept_id, source_value,
	...
	select 	domain_id,
		...
		visit_detail_id,
		provider_id,
		row_number() over (order by person_id, start_date, end_date) + cte2.start_id as id,
		concept_id,
		source_value,
		source_concept_id,
		type_concept_id,
