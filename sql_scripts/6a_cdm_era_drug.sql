...
(
	...
	FROM {TARGET_SCHEMA}.drug_exposure d
	     JOIN {VOCABULARY_SCHEMA}.concept_ancestor ca ON ca.descendant_concept_id = d.drug_concept_id
	     JOIN {VOCABULARY_SCHEMA}.concept c ON ca.ancestor_concept_id = c.concept_id
		 JOIN (SELECT vocabulary_id FROM (VALUES ('RxNorm'), ('RxNorm Extension')) AS voc1 (vocabulary_id)) as v1 on c.vocabulary_id = v1.vocabulary_id --AD: added 23/05/2023
		 LEFT JOIN {VOCABULARY_SCHEMA}.concept c2 
			ON lower(d.dose_unit_source_value) = lower(c2.concept_name) 
			AND c2.vocabulary_id in ('RxNorm', 'RxNorm Extension') --AD: Added RxNorm Extension on 23/05/2023
			AND c2.concept_class_id = 'Dose Form' 
	     WHERE --c.vocabulary_id in ('RxNorm', 'RxNorm Extension') --AD: Added "RxNorm Extension" and transformed in a JOIN to avoid OR which is slow 23/05/2023
	     	/*AND*/ c.concept_class_id = 'Ingredient'
	     /* Depending on the needs of your data, you can put more filters on to your code. We assign 0 to unmapped 's, and we found data where days_supply was negative.
	      * We don't want different drugs put in the same era, so the code below shows how we filtered them out.
	      * We also don't want negative days_supply, because that will pull our end_date before the start_date due to our second parameter in the COALESCE function.
	      * For now, we are filtering those out as well, but this is a ddrug_concept_idata quality issue that we are trying to solve.
	      */
	     AND d.drug_concept_id != 0
	     AND d.days_supply >= 0
)
...
-----------------------------------------------------------------------------------------------------------------------------
INSERT INTO {TARGET_SCHEMA}.dose_era(dose_era_id, person_id, drug_concept_id, unit_concept_id, dose_value, dose_era_start_date, dose_era_end_date)
SELECT
 	row_number() over (order by person_id, drug_concept_id)
	, person_id
	, drug_concept_id
...
--------------------------------
-- Add PK / IDX / FK to DOSE_ERA
--------------------------------
ALTER TABLE {TARGET_SCHEMA}.dose_era  ADD CONSTRAINT xpk_dose_era PRIMARY KEY (dose_era_id);

CREATE INDEX idx_dose_era_person_id  ON {TARGET_SCHEMA}.dose_era (person_id ASC);
CLUSTER {TARGET_SCHEMA}.dose_era USING idx_dose_era_person_id;

CREATE INDEX idx_dose_era_concept_id ON {TARGET_SCHEMA}.dose_era (drug_concept_id ASC);

ALTER TABLE {TARGET_SCHEMA}.dose_era ADD CONSTRAINT fpk_dose_era_person FOREIGN KEY (person_id) REFERENCES {TARGET_SCHEMA}.person (person_id);

ALTER TABLE {TARGET_SCHEMA}.dose_era ADD CONSTRAINT fpk_dose_era_concept FOREIGN KEY (drug_concept_id) REFERENCES {VOCABULARY_SCHEMA}.concept (concept_id);

ALTER TABLE {TARGET_SCHEMA}.dose_era ADD CONSTRAINT fpk_dose_era_unit_concept FOREIGN KEY (unit_concept_id) REFERENCES {VOCABULARY_SCHEMA}.concept (concept_id);
